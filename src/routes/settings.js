...
router.get('/', isAuthenticated, async (req, res) => {
    try {
        const map = await SettingsService.getMany(['webhook_url', 'webhook_secret', 'webhook_toggle_connection', 'webhook_toggle_message_in', 'webhook_toggle_message_out']);
...
router.post('/webhook', isAuthenticated, async (req, res) => {
    ...
    try {
        await SettingsService.set('webhook_url', webhook_url || '', 'Webhook endpoint URL');
        await SettingsService.set('webhook_secret', webhook_secret || '', 'Webhook HMAC secret');
        await SettingsService.set('webhook_toggle_connection', webhook_toggle_connection ? 'true' : 'false', 'Toggle connection status webhook');
        await SettingsService.set('webhook_toggle_message_in', webhook_toggle_message_in ? 'true' : 'false', 'Toggle incoming message webhook');
        await SettingsService.set('webhook_toggle_message_out', webhook_toggle_message_out ? 'true' : 'false', 'Toggle outgoing message webhook');
